...
function planVsActualMarkup(a){
  ...
  const ticks=Array.from({length:21},(_,i)=>{const t=(-90+i*9)*Math.PI/180,r1=116,r2=i%5===0?101:108,cx=180,cy=170;return `<line x1="${cx+Math.cos(t)*r1}" y1="${cy+Math.sin(t)*r1}" x2="${cx+Math.cos(t)*r2}" y2="${cy+Math.sin(t)*r2}"/>`;}).join('');
  return `<div class="production-plan-card odometer-premium"><div class="production-gauge-title-row"><div><div class="production-gauge-title">Plan vs Actual</div><div class="gauge-subtitle">Production achievement</div></div>${kpiGear('plan_actual')}</div><div class="odometer reference premium redesigned"><svg viewBox="0 0 360 225" role="img" aria-label="Plan vs Actual ${planned?achievement.toFixed(1):'N/A'} percent"><defs><linearGradient id="odoNeedle" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#143980"/><stop offset="100%" stop-color="#0cc0df"/></linearGradient></defs><path d="M 35 170 A 145 145 0 0 1 325 170" fill="none" stroke="#e8eff2" stroke-width="30"/><path d="M 35 170 A 145 145 0 0 1 325 170" fill="none" stroke="#ff3131" stroke-width="27" pathLength="100" stroke-dasharray="70 30"/><path d="M 35 170 A 145 145 0 0 1 325 170" fill="none" stroke="#f0b323" stroke-width="27" pathLength="100" stroke-dasharray="10 90" stroke-dashoffset="-70"/><path d="M 35 170 A 145 145 0 0 1 325 170" fill="none" stroke="#16a957" stroke-width="27" pathLength="100" stroke-dasharray="20 80" stroke-dashoffset="-80"/><g class="odometer-tick-lines">${ticks}</g><g class="odometer-needle redesigned" style="transform:rotate(${angle}deg)"><line x1="180" y1="170" x2="180" y2="61"/><circle cx="180" cy="170" r="15"/></g><circle cx="180" cy="170" r="7" fill="#fff"/><rect x="122" y="123" width="116" height="45" rx="14" fill="#fff" stroke="#e1eaee"/><text x="180" y="145" text-anchor="middle" class="odometer-value">${planned?achievement.toFixed(1)+'%':'—'}</text><text x="180" y="158" text-anchor="middle" class="odometer-label">ACHIEVEMENT</text><text x="35" y="198" text-anchor="middle" class="odometer-scale-label">0%</text><text x="238" y="198" text-anchor="middle" class="odometer-scale-label">70%</text><text x="325" y="198" text-anchor="middle" class="odometer-scale-label">100%</text></svg></div><div class="plan-actual-values premium"><div><span>Planned</span><strong>${planned?Math.round(planned).toLocaleString():'N/A'}</strong></div><div><span>Actual</span><strong>${actual.toLocaleString()}</strong></div><div><span>Achievement</span><strong>${planned?achievement.toFixed(1)+'%':'N/A'}</strong></div></div><small class="gauge-note">Planned downtime is excluded from available production time.</small></div>`;
}
