...
function productionGaugeMarkup(a){
  ...
  const rings=[{r:112,w:22},{r:84,w:20},{r:56,w:18}];
  const paths=rings.map((ring,i)=>`<circle class="oee-full-track" cx="160" cy="160" r="${ring.r}" stroke-width="${ring.w}"/><circle class="oee-full-value" cx="160" cy="160" r="${ring.r}" stroke="${colors[i]}" stroke-width="${ring.w}" stroke-dasharray="${2*Math.PI*ring.r}" stroke-dashoffset="${2*Math.PI*ring.r*(1-vals[i])}"/>`).join('');
  return `<div class="production-gauge-card"><div class="production-gauge-title">OEE</div><div class="oee-ring-wrap full"><svg class="oee-ring" viewBox="0 0 320 320" role="img" aria-label="OEE ${gaugeValue(a.oee)}"><g transform="rotate(-90 160 160)">${paths}</g></svg><div class="oee-ring-center"><strong>${gaugeValue(a.oee)}</strong><span>Overall Equipment Effectiveness</span></div></div><div class="gauge-legend">${labels.map((l,i)=>`<span><i class="gauge-dot" style="background:${colors[i]}"></i>${l} <b>${gaugeValue([a.availability,a.performance,a.quality][i])}</b></span>`).join('')}</div></div>`;
}
