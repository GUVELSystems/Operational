...
async function renderStatusFoundation(){
  ...
  view.innerHTML=`<section class="page-head"><div><div class="eyebrow">GUVEL OPERATIONAL · PHASE 2.0.C</div><h1>Status</h1><p>Live machine status and production session control.</p></div><div class="capture-live-badge"><span></span>${active.length} active machine session(s)</div></section><section class="status-section"><div class="section-title status-section-title"><div><h2>Machine Status</h2><p>Current state of each machine.</p></div></div><div class="status-machine-grid" id="statusMachineGrid"></div></section><section class="panel status-session-panel"><div class="section-title"><div><h2>Start production session</h2><p>One active production session per machine. Active machines cannot be started twice.</p></div></div><form id="statusStartForm" class="form-grid"><div class="field"><label>Machine *</label><select id="stMachine" required><option value="">Select machine</option>${machines.map(x=>`<option value="${x.id}" ${activeMachineIds.has(x.id)?'disabled':''}>${escapeHtml(x.code)} — ${escapeHtml(x.name||'')}${activeMachineIds.has(x.id)?' — RUNNING':''}</option>`).join('')}</select></div><div class="field"><label>Shift *</label><select id="stShift" required><option value="">Select shift</option>${shifts.map(x=>`<option value="${x.id}">${escapeHtml(x.code)} — ${escapeHtml(x.name||'')}</option>`).join('')}</select></div><div class="field"><label>Customer *</label><select id="stCustomer" required><option value="">Select customer</option>${customers.map(x=>`<option value="${x.id}">${escapeHtml(x.code)} — ${escapeHtml(x.name||'')}</option>`).join('')}</select></div><div class="field"><label>Part Number *</label><select id="stPart" required><option value="">Select part number</option>${parts.map(x=>`<option value="${x.id}" data-customer="${x.customer_id}">${escapeHtml(x.part_number)} — ${escapeHtml(x.description||'')}</option>`).join('')}</select></div><div class="field"><label>Lot Number *</label><input id="stLot" required maxlength="120" autocomplete="off"></div><div class="field status-operation-field"><label>Operation <span id="stOperationRequiredHint">(select part number)</span></label><select id="stOperation" disabled><option value="">Select operation</option></select><small id="stOperationHint" class="field-hint">Select a part number to load its operations.</small></div><div class="field"><label>Operator</label><select id="stOperator"><option value="">Select operator</option>${personnel.filter(x=>x.role==='Operator').map(x=>`<option value="${x.id}">${escapeHtml(fullName(x))}</option>`).join('')}</select></div><div class="field"><label>Supervisor</label><select id="stSupervisor"><option value="">Select supervisor</option>${personnel.filter(x=>x.role==='Supervisor').map(x=>`<option value="${x.id}">${escapeHtml(fullName(x))}</option>`).join('')}</select></div><div class="form-actions"><button class="primary" id="statusStartButton" type="submit">Start Production</button><div id="statusMsg" class="status" role="status" aria-live="polite"></div></div></form></section>`;
  const grid=document.getElementById('statusMachineGrid');
  const stMachine=document.getElementById('stMachine');
  const stPart=document.getElementById('stPart');
  const stCustomer=document.getElementById('stCustomer');
  const stOperation=document.getElementById('stOperation');
  const stOperationHint=document.getElementById('stOperationHint');
  const stOperationRequiredHint=document.getElementById('stOperationRequiredHint');
  const renderCards=()=>{grid.innerHTML=machines.map(machine=>{const s=active.find(x=>x.machine_id===machine.id);return `<article class="status-machine-card ${s?'is-running':'is-idle'}"><div class="status-machine-top"><div><div class="eyebrow">${escapeHtml(machine.code)}</div><h2>${escapeHtml(machine.name||'Machine')}</h2></div><span class="status-state">${s?'RUNNING':'IDLE'}</span></div>${s?`<div class="status-session-info"><strong>${escapeHtml(s.part_numbers?.part_number||'Part number')}</strong><span>Lot: ${escapeHtml(s.lot_number||'—')}</span><span>Shift: ${escapeHtml(s.shifts?.code||'—')}</span><span>Operation: ${escapeHtml(s.operations?.operation_number||'—')}</span><span>Started: ${s.started_at?new Date(s.started_at).toLocaleString():'—'}</span></div><button class="danger status-finish" data-id="${s.id}" data-machine="${escapeHtml(machine.code)}">Finish Session</button>`:'<p class="muted">No active production session.</p>'}</article>`}).join('');grid.querySelectorAll('.status-finish').forEach(b=>b.onclick=async()=>{if(!confirm(`Finish the production session on ${b.dataset.machine}?`))return;b.disabled=true;b.textContent='Finishing…';const r=await sb.from('machine_production_sessions').update({status:'finished',finished_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',b.dataset.id).eq('company_id',activeCompanyId);if(r.error){b.disabled=false;b.textContent='Finish Session';return alert(r.error.message);}await renderStatusFoundation();});};
  const refreshOperationOptions=()=>{const partId=stPart.value;const machineId=stMachine.value;stOperation.innerHTML='<option value="">Select operation</option>';stOperation.disabled=!partId;stOperation.required=false;if(!partId){stOperationHint.textContent='Select a part number to load its operations.';stOperationRequiredHint.textContent='(select part number)';return;}let ops=operationList.filter(x=>x.part_number_id===partId);const machineOps=operationMachineLinks.filter(x=>x.part_number_id===partId&&x.machine_id===machineId).map(x=>x.operation_id);if(machineId&&machineOps.length){ops=ops.filter(x=>machineOps.includes(x.id));stOperationHint.textContent='Showing operations configured for this part number and machine.';}else if(machineId){stOperationHint.textContent='No machine-specific operation mapping found; showing all part-number operations.';}if(!ops.length){stOperationHint.textContent='No operations configured for this part number.';stOperationRequiredHint.textContent='(not configured)';return;}stOperation.required=true;stOperationRequiredHint.textContent='*';stOperation.innerHTML='<option value="">Select operation</option>'+ops.map(x=>`<option value="${x.id}">${escapeHtml(x.operation_number)} — ${escapeHtml(x.operation_name||'')}</option>`).join('');};
  const syncCustomerFromPart=()=>{const selected=stPart.options[stPart.selectedIndex];if(!selected||!selected.dataset.customer)return;const customerId=selected.dataset.customer;if(!stCustomer.value)stCustomer.value=customerId;};
  renderCards();
  stMachine.addEventListener('change',refreshOperationOptions);stPart.addEventListener('change',()=>{syncCustomerFromPart();refreshOperationOptions();});
  document.getElementById('statusStartForm').onsubmit=async e=>{e.preventDefault();const msg=document.getElementById('statusMsg');const button=document.getElementById('statusStartButton');const machineId=stMachine.value;if(!machineId||activeMachineIds.has(machineId)){msg.textContent='Select an available machine. This machine already has an active session.';msg.className='status error';return;}const payload={company_id:activeCompanyId,machine_id:machineId,shift_id:document.getElementById('stShift').value,customer_id:stCustomer.value,part_number_id:stPart.value,lot_number:document.getElementById('stLot').value.trim(),operation_id:stOperation.value||null,operator_id:document.getElementById('stOperator').value||null,supervisor_id:document.getElementById('stSupervisor').value||null};if(!payload.lot_number){msg.textContent='Lot number is required.';msg.className='status error';return;}if(stOperation.required&&!payload.operation_id){msg.textContent='Select an operation for this part number.';msg.className='status error';return;}button.disabled=true;button.textContent='Starting…';msg.textContent='';const r=await sb.from('machine_production_sessions').insert(payload);if(r.error){button.disabled=false;button.textContent='Start Production';msg.textContent=r.error.message;msg.className='status error';return;}await renderStatusFoundation();};
}
