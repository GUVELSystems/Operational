...
function dashboard(){const tabs=['General','Production','Quality','Downtime','Plant Now'];return `<div class="page-head dashboard-page-head"><div><div class="eyebrow">GUVEL OPERATIONAL</div><h1>Dashboard</h1></div><div class="page-head-actions"><button id="dashboardFullscreen" class="fullscreen-btn" type="button">⛶ Full Screen</button></div></div><div class="tabs dashboard-tabs">${tabs.map((x,i)=>`<button class="tab ${i===0?'active':''}" data-dashboard-tab="${x}">${x}</button>`).join('')}</div><div id="dashboardFilterBackdrop" class="dashboard-filter-backdrop" hidden></div><div id="dashboardFilters" class="dashboard-filter-popover" role="dialog" aria-labelledby="dashboardFiltersTitle" hidden><div class="dfp-head"><h2 id="dashboardFiltersTitle">Filters</h2><button id="dashboardFiltersClose" class="icon-button" type="button" aria-label="Close filters">×</button></div><div class="form-grid dashboard-filters"><div class="field period-field"><label>Period</label><select id="dashPeriod">${DASH_PERIODS.map(x=>`<option value="${x}" ${dashboardState.filters.period===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="field"><label>Date From</label><input id="dashFrom" type="date"></div><div class="field"><label>Date To</label><input id="dashTo" type="date"></div><div class="field"><label>Customer</label><select id="dashCustomer"><option value="">All Customers</option></select></div><div class="field"><label>Part Number</label><select id="dashPart"><option value="">All Part Numbers</option></select></div><div class="field"><label>Shift</label><select id="dashShift"><option value="">All Shifts</option></select></div><div class="field"><label>Machine</label><select id="dashMachine"><option value="">All Machines</option></select></div></div><div id="dashboardPeriodHint" class="dashboard-period-hint" hidden></div><div id="dashboardStatus" class="status"></div><div class="dfp-foot"><button id="dashboardClear" class="link-btn" type="button">Clear filters</button><div class="actions"><button id="dashboardRefresh" class="secondary" type="button">Refresh data</button><button id="dashboardFiltersDone" class="primary" type="button">Done</button></div></div></div><div id="dash"></div>`}
function dashTab(t){dashboardState.tab=t;document.documentElement.dataset.dashboardTab=t;const dash=document.getElementById('dash');if(!dash)return;/* Phase 3.0.D: hold the current height while the new tab renders so the page does not jump. */dash.style.minHeight=dash.offsetHeight+'px';requestAnimationFrame(()=>requestAnimationFrame(()=>{dash.style.minHeight='';}));if(t==='General'){dash.innerHTML=`<div id="dashboardGeneral"></div>`;renderDashboardGeneral();return;}if(t==='Production'){dash.innerHTML=`<div id="dashboardProduction"></div>`;renderProductionDashboard();return;}if(t==='Quality'){dash.innerHTML=`<div id="dashboardQuality"></div>`;renderQualityDashboard();return;}if(t==='Downtime'){dash.innerHTML=`<div id="dashboardDowntime"></div>`;renderDowntimeDashboard();return;}if(t==='Plant Now'){dash.innerHTML=`<div id="dashboardPlantNow"></div>`;renderPlantNowDashboard();return;}dash.innerHTML=`<div class="panel dashboard-coming"><div class="eyebrow">PHASE 1.9</div><h2>${escapeHtml(t)} Dashboard</h2><p>This section is reserved for the next Dashboard subphase.</p><span class="dashboard-phase-badge">Implementation deferred</span></div>`;}
...
function dashPct2(v){return v==null?'N/A':`${(v*100).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}%`;}
function dashNum(v){return Number(v||0).toLocaleString(undefined,{maximumFractionDigits:2});}
/* Phase 2.0.H.14 — KPI content fit: compact only when the full value is longer than 9 characters; full value stays in the tooltip. */
function dashFit(v,fmt){const full=fmt(v);if(v==null||full.length<=9)return full;return Number(v).toLocaleString(undefined,{notation:'compact',maximumFractionDigits:2});}
function dashFitTitle(v,fmt){return ` title="${escapeHtml(fmt(v))}"`;}
function dashPpmText(v){return v==null?'N/A':Math.round(v).toLocaleString();}
...
/* Phase 3.0.A — theme bridge: charts read the design tokens so they follow the active theme. */
function guvelToken(name){return getComputedStyle(document.documentElement).getPropertyValue(name).trim();}
function guvelSeries(){return ['--chart-1','--chart-2','--chart-3','--chart-4','--chart-5','--chart-6'].map(guvelToken);}
const GUVEL_LEGACY_COLORS={'#0cc0df':'--chart-1','#ff3131':'--chart-2','#143980':'--chart-2','#16a957':'--chart-3','#006732':'--chart-3','#00a878':'--chart-3','#21a179':'--chart-3','#f2bd21':'--chart-4','#f4b400':'--chart-4','#f0a24b':'--chart-4','#ef7d32':'--chart-4','#7b61ff':'--chart-5','#8c6bb1':'--chart-5','#5b7083':'--chart-6','#ffffff':'--surface','#dce5e9':'--line-soft'};
...
function dashboardPrefs(){try{const p=JSON.parse(localStorage.getItem(dashboardPrefsKey())||'{}');return {...p,kpis:p.kpis||{},charts:p.charts||{}};}catch{return {kpis:{},charts:{}};}}
...
function kpiColorFor(key,value){if(value==null)return '';const c=dashboardPrefs().kpis?.[key]||{},b=c.between,g=c.greater,l=c.less;if(b?.enabled&&b.min!=null&&b.max!=null&&value>=Math.min(b.min,b.max)&&value<=Math.max(b.min,b.max))return b.color||'';if(g?.enabled&&g.value!=null&&value>=g.value)return g.color||'';if(l?.enabled&&l.value!=null&&value<=l.value)return l.color||'';return '';}
function kpiThresholdColor(key,value,fallback){return kpiColorFor(key,value)||fallback;}
function thresholdSegments(key){const c=dashboardPrefs().kpis?.[key]||{},b=c.between,g=c.greater,l=c.less,segments=[];if(l?.enabled&&l.value!=null)segments.push({from:0,to:Math.max(0,Math.min(1,l.value)),color:l.color||'#ff3131'});if(b?.enabled&&b.min!=null&&b.max!=null){const from=Math.max(0,Math.min(1,Math.min(b.min,b.max))),to=Math.max(0,Math.min(1,Math.max(b.min,b.max)));segments.push({from,to,color:b.color||'#f2bd21'});}if(g?.enabled&&g.value!=null)segments.push({from:Math.max(0,Math.min(1,g.value)),to:1,color:g.color||'#16a957'});if(!segments.length)segments.push({from:0,to:.7,color:'var(--stop)'},{from:.7,to:.8,color:'var(--watch)'},{from:.8,to:1,color:'var(--flow)'});return segments.filter(x=>x.to>x.from); }
function kpiStatusStyle(key,value){const color=kpiColorFor(key,value);return /^#[0-9a-fA-F]{6}$/.test(color)?`style="--kpi-status:${color}" data-kpi-status="active"`:'';}
...
function renderActiveDashboard(){if(dashboardState.tab==='Production')renderProductionDashboard();else if(dashboardState.tab==='Quality')renderQualityDashboard();else if(dashboardState.tab==='Downtime')renderDowntimeDashboard();else renderDashboardGeneral();requestAnimationFrame(()=>animateDashboardKpis());}
function saveKpiSettings(key){const read=id=>{const el=document.getElementById(id);if(!el)throw new Error('KPI configuration control not found: '+id);const v=el.value??'';return kpiInputToStored(key,v);};const p=dashboardPrefs();p.kpis[key]={greater:{enabled:document.getElementById('kpiGreaterEnabled').checked,value:read('kpiGreaterValue'),color:document.getElementById('kpiGreaterColor').value},less:{enabled:document.getElementById('kpiLessEnabled').checked,value:read('kpiLessValue'),color:document.getElementById('kpiLessColor').value},between:{enabled:document.getElementById('kpiBetweenEnabled').checked,min:read('kpiBetweenMin'),max:read('kpiBetweenMax'),color:document.getElementById('kpiBetweenColor').value},...(key==='prod_oee'?{availabilityColor:document.getElementById('oeeAvailabilityColor')?.value||'#0cc0df',performanceColor:document.getElementById('oeePerformanceColor')?.value||'#16a957',qualityColor:document.getElementById('oeeQualityColor')?.value||'#ff3131'}:{})};saveDashboardPrefs(p);closeDashboardConfig();renderActiveDashboard();}
...
function chartReferenceDatasets(key,labels){const c=dashboardPrefs().charts?.[key]||{},make=(v,label,dash)=>v==null?null:{label,data:labels.map(()=>v*(DASH_CHART_META[key].unit==='%'?100:1)),borderColor:label==='Objetivo'?guvelToken('--flow'):guvelToken('--chart-alert'),borderWidth:label==='Objetivo'?2:1.5,borderDash:dash,pointRadius:0,fill:false};return [make(c.min,'Min',[6,4]),make(c.target,'Objetivo',[3,3]),make(c.max,'Max',[6,4])].filter(Boolean);}
...
function gaugeValue(v){return v==null?'N/A':`${(v*100).toFixed(1)}%`;}
function oeeArcPath(cx,cy,r,startDeg,endDeg){const rad=d=>d*Math.PI/180;const p1=[cx+r*Math.cos(rad(startDeg)),cy+r*Math.sin(rad(startDeg))],p2=[cx+r*Math.cos(rad(endDeg)),cy+r*Math.sin(rad(endDeg))];return `M ${p1[0].toFixed(2)} ${p1[1].toFixed(2)} A ${r} ${r} 0 0 1 ${p2[0].toFixed(2)} ${p2[1].toFixed(2)}`;}
function productionGaugeMarkup(a){
  const vals=[a.availability,a.performance,a.quality].map(v=>v==null?0:Math.max(0,Math.min(1,Number(v))));
  const pref=dashboardPrefs().kpis?.prod_oee||{};
  const colors=[pref.availabilityColor||'var(--chart-1)',pref.performanceColor||'var(--chart-2)',pref.qualityColor||'var(--chart-3)'],labels=['Availability','Performance','Quality'];
  const rings=[{r:126,w:18},{r:99,w:18},{r:72,w:18}];
  const paths=rings.map((ring,i)=>{const c=2*Math.PI*ring.r;const p=c*vals[i];return `<circle class="guvel-oee-track" cx="160" cy="160" r="${ring.r}"/><circle class="guvel-oee-value" cx="160" cy="160" r="${ring.r}" style="stroke:${colors[i]}" stroke-dasharray="${c.toFixed(2)}" stroke-dashoffset="${c.toFixed(2)}" data-progress="${vals[i]}"/>`;}).join('');
...
function planVsActualMarkup(a){
  let planned=0,actual=a.production||0;
  for(const r of a.d.prod){const sh=r.shifts,dur=shiftDurationSeconds(sh),excluded=Number(sh?.excluded_planned_minutes||0)*60,available=dur==null?0:Math.max(0,dur-excluded),key=`${r.part_number_id||'unknown-part'}|${r.operation_id||'unknown-operation'}|${r.machine_id||'unknown-machine'}`,cfg=dashboardState.cycleTimes.find(x=>x.key===key),cy=Number(cfg?.cycle_time_seconds||r.operations?.ideal_cycle_time_seconds||0);if(available>0&&cy>0)planned+=available/cy;}
  const achievement=planned>0?Math.max(0,actual/planned):0,clamped=Math.min(1,achievement),angle=-90+clamped*180;
  const polar=(deg,r)=>{const rad=deg*Math.PI/180;return [180+Math.cos(rad)*r,190+Math.sin(rad)*r];};
  const arc=(start,end,r)=>{const a=polar(start,r),b=polar(end,r);return `M ${a[0].toFixed(2)} ${a[1].toFixed(2)} A ${r} ${r} 0 0 1 ${b[0].toFixed(2)} ${b[1].toFixed(2)}`;};
  const ticks=Array.from({length:21},(_,i)=>{const d=180+i*9,outer=145,inner=i%5===0?130:137,a=polar(d,outer),b=polar(d,inner);return `<line x1="${a[0]}" y1="${a[1]}" x2="${b[0]}" y2="${b[1]}"/>`;}).join('');
  const segments=thresholdSegments('plan_actual');
  const zones=segments.map(seg=>`<path d="${arc(180+seg.from*180,180+seg.to*180,132)}" class="guvel-odo-zone" style="stroke:${({'#ff3131':'var(--stop)','#f2bd21':'var(--watch)','#16a957':'var(--flow)','#0cc0df':'var(--flow)'})[String(seg.color||'').toLowerCase()]||seg.color}"/>`).join('');
  const needleColor=kpiThresholdColor('plan_actual',achievement,achievement<.7?'var(--stop)':achievement<.8?'var(--watch)':'var(--flow)');
  const value=planned?achievement*100:'—';
  const display=planned?Number(value).toFixed(1)+'%':'—';
  return `<div class="production-plan-card guvel-gauge-card guvel-plan-card"><div class="production-gauge-title-row"><div><div class="production-gauge-title">Plan vs Actual</div><div class="gauge-subtitle">Production achievement</div></div>${kpiGear('plan_actual')}</div><div class="guvel-odo-visual"><svg viewBox="0 0 360 250" role="img" aria-label="Plan vs Actual ${display}"><path d="${arc(180,360,132)}" class="guvel-odo-track"/>${zones}<g class="guvel-odo-ticks">${ticks}</g><g class="guvel-odo-needle" data-angle="${angle}" style="transform:rotate(${angle}deg);--needle:${needleColor}"><line x1="180" y1="190" x2="180" y2="75"/><circle cx="180" cy="190" r="14"/></g><circle cx="180" cy="190" r="7" class="guvel-odo-hub"/><text x="180" y="145" text-anchor="middle" class="guvel-odo-value" data-value="${planned?achievement*100:''}">${display}</text><text x="180" y="162" text-anchor="middle" class="guvel-odo-label">ACHIEVEMENT</text><text x="42" y="226" text-anchor="middle" class="guvel-odo-scale">0%</text><text x="180" y="35" text-anchor="middle" class="guvel-odo-scale">50%</text><text x="318" y="226" text-anchor="middle" class="guvel-odo-scale">100%</text></svg></div><div class="plan-actual-values premium"><div><span>Planned</span><strong>${planned?Math.round(planned).toLocaleString():'N/A'}</strong></div><div><span>Actual</span><strong>${actual.toLocaleString()}</strong></div><div><span>Achievement</span><strong>${display}</strong></div></div><small class="gauge-note">Planned downtime is excluded from available production time.</small></div>`;
}
function productionPlanListMarkup(a){
  const groups=new Map();
  for(const r of a.d.prod){const customer=dashboardState.customers.find(c=>c.id===r.customer_id);const customerLabel=customer?`${customer.code} — ${customer.name}`:'—';const partLabel=r.part_numbers?.part_number||dashboardState.parts.find(x=>x.id===r.part_number_id)?.part_number||'—';const key=`${customerLabel}|${partLabel}`;let g=groups.get(key);if(!g){g={customer:customerLabel,part:partLabel,planned:0,actual:0};groups.set(key,g);}g.actual+=Number(r.production_quantity||0);const sh=r.shifts,dur=shiftDurationSeconds(sh),excluded=Number(sh?.excluded_planned_minutes||0)*60,available=dur==null?0:Math.max(0,dur-excluded),cfg=dashboardState.cycleTimes.find(x=>x.key===`${r.part_number_id||'unknown-part'}|${r.operation_id||'unknown-operation'}|${r.machine_id||'unknown-machine'}`),cy=Number(cfg?.cycle_time_seconds||r.operations?.ideal_cycle_time_seconds||0);if(available>0&&cy>0)g.planned+=available/cy;}
  const rows=[...groups.values()].sort((x,y)=>y.actual-x.actual);
  return `<div class="production-plan-list"><div class="production-gauge-title">Production Plan vs Real</div><div class="production-table-wrap"><table class="production-plan-table"><thead><tr><th>Cliente</th><th>Número de Parte</th><th>Cantidad Planeada</th><th>Cantidad Real</th><th>% Cumplimiento</th></tr></thead><tbody>${rows.map(g=>`<tr><td>${escapeHtml(g.customer)}</td><td>${escapeHtml(g.part)}</td><td>${g.planned?Math.round(g.planned).toLocaleString():'N/A'}</td><td>${g.actual.toLocaleString()}</td><td><span class="achievement-pill ${g.planned&&g.actual/g.planned>=.8?'good':g.planned&&g.actual/g.planned>=.7?'warn':'bad'}">${g.planned?(g.actual/g.planned*100).toFixed(1)+'%':'N/A'}</span></td></tr>`).join('')||'<tr><td colspan="5">No production plan data.</td></tr>'}</tbody></table></div></div>`;
}
function productionMetricTile(label,value,kind){return `<div class="production-metric-tile ${kind}"><span>${label}</span><strong>${value}</strong></div>`;}
function liveLineStatusMarkup(a,p){
  const dayOnly=['Today','Previous Day','Yesterday'].includes(p.label);
  if(dayOnly){const activeIds=new Set((dashboardState.sessions||[]).filter(s=>['RUNNING','ACTIVE','IN_PROGRESS'].includes(String(s.status||'').toUpperCase())).map(s=>s.machine_id));const active=dashboardState.machines.filter(m=>activeIds.has(m.id));if(!active.length)return '';return `<div class="live-line-status"><div class="live-line-head"><h3>Live Line Status</h3><span>${active.length} active</span></div><div class="live-line-grid">${active.map(m=>`<div class="live-line-item active"><i></i><span>${escapeHtml(m.code||m.name||'Machine')}</span><b>ACTIVE</b></div>`).join('')}</div></div>`;}
  const byDay=new Map();for(const r of a.d.prod){const day=String(r.production_date||'').slice(0,10);if(day)byDay.set(day,(byDay.get(day)||new Set()).add(r.machine_id));}const rows=[...byDay.entries()].sort((x,y)=>x[0].localeCompare(y[0]));const max=Math.max(1,...rows.map(([,set])=>set.size));const points=rows.map(([day,set],i)=>`${rows.length===1?50:i/(rows.length-1)*260+20},${110-set.size/max*82}`).join(' ');return `<div class="live-line-status"><div class="live-line-head"><h3>Live Line Status</h3><span>Machines per day</span></div>${rows.length?`<div class="line-trend-chart"><svg viewBox="0 0 300 132" role="img" aria-label="Trend of active machines by day"><line x1="20" y1="110" x2="280" y2="110"/><polyline points="${points}"/><g>${rows.map(([day,set],i)=>{const x=rows.length===1?50:i/(rows.length-1)*260+20;return `<text x="${x}" y="128" text-anchor="middle">${escapeHtml(day.slice(5))}</text><circle cx="${x}" cy="${110-set.size/max*82}" r="3.5"/>`;}).join('')}</g></svg></div>`:'<p class="muted">No daily machine activity.</p>'}</div>`;
}
function modelMixMarkup(a){const mix=new Map();for(const r of a.d.prod){const key=r.part_number_id||'unknown';const label=r.part_numbers?.part_number||key;mix.set(label,(mix.get(label)||0)+Number(r.production_quantity||0));}const rows=[...mix.entries()].sort((x,y)=>y[1]-x[1]);const total=rows.reduce((n,x)=>n+x[1],0);let acc=0;const stops=rows.map(([label,v],i)=>{const start=acc;acc+=total?v/total*100:0;return `${['var(--chart-1)','var(--chart-2)','var(--chart-3)','var(--chart-4)','var(--chart-5)','var(--chart-6)'][i%6]} ${start}% ${acc}%`;}).join(',');return `<div class="model-mix-card"><div class="production-gauge-title">Model Mix</div><div class="pie-mix" style="background:conic-gradient(${stops||'var(--line-soft) 0 100%'})"></div><div class="mix-list">${rows.slice(0,6).map(([label,v],i)=>`<div><i style="background:${['var(--chart-1)','var(--chart-2)','var(--chart-3)','var(--chart-4)','var(--chart-5)','var(--chart-6)'][i%6]}"></i><span>${escapeHtml(label)}</span><b>${total?(v/total*100).toFixed(1):0}%</b></div>`).join('')||'<p class="muted">No production data.</p>'}</div></div>`;}
...
/* Phase 3.0.B — Plan vs Actual needle sweeps from 0% to the result; the figure counts up with it. */
function animatePlanNeedle(){
  const needle=document.querySelector('.guvel-odo-needle[data-angle]');if(!needle)return;
  const target=Number(needle.dataset.angle),valEl=document.querySelector('.guvel-odo-value[data-value]'),raw=valEl?.dataset.value,finalVal=raw===''||raw==null?null:Number(raw);
  const setAt=(e)=>{needle.style.transform=`rotate(${-90+(target+90)*e}deg)`;if(valEl&&finalVal!=null&&isFinite(finalVal))valEl.textContent=(finalVal*e).toFixed(1)+'%';};
  needle.style.transition='none';
  if(!isFinite(target)||window.matchMedia('(prefers-reduced-motion: reduce)').matches){setAt(1);return;}
  setAt(0);const dur=1600,t0=performance.now(),ease=t=>1-Math.pow(1-t,3);
  const step=now=>{if(!needle.isConnected)return;const k=Math.min(1,(now-t0)/dur);setAt(ease(k));if(k<1)requestAnimationFrame(step);};
...
function renderProductionDashboard(){
  ...
  if(!a.d.prod.length){box.innerHTML='<div class="panel dashboard-empty"><div class="eyebrow">PRODUCTION</div><h2>No Data</h2><p>No production captures match the selected Dashboard filters.</p><div class="dashboard-empty-actions">'+dashboardFilterButton()+'</div></div>';dashboardStatus('No Data in the selected filter scope.');return;}
  ...
  box.innerHTML=`${warning}<div class="dashboard-overview-head"><div><div class="eyebrow">PRODUCTION OVERVIEW</div><h2>${escapeHtml(p.label)}</h2></div><div class="dashboard-overview-actions"><div class="dashboard-live-badge"><span></span>Production intelligence</div>${dashboardFilterButton()}</div></div>
  <div class="production-command-grid dashboard-module-animate">
    ${productionGaugeMarkup(a)}
    ${planVsActualMarkup(a)}
    <div class="production-metric-stack">${productionMetricTile('Good Parts',dashNum(a.good),'good')}${productionMetricTile('Rejected Parts',dashNum(a.scrap),'rejected')}${productionMetricTile('Yield / FTQ',dashPct(a.production>0?a.good/a.production:null),'yield')}</div>
    ${productionPlanListMarkup(a)}
    ${modelMixMarkup(a)}
  </div>`;
  dashboardStatus(`${a.d.prod.length.toLocaleString()} production captures · ${a.production.toLocaleString()} pieces produced.`);updateDashboardPeriodHint(p);renderProductionCharts(cmp);animateProductionVisuals();
}
...
function renderDowntimeDashboard(){
  ...
  if(!a.d.prod.length){box.innerHTML='<div class="panel dashboard-empty"><div class="eyebrow">DOWNTIME</div><h2>No Data</h2><p>No production captures match the selected Dashboard filters.</p><div class="dashboard-empty-actions">'+dashboardFilterButton()+'</div></div>';dashboardStatus('No Data in the selected filter scope.');return;}
  ...
  box.innerHTML=`${warning}<div class="dashboard-overview-head"><div><div class="eyebrow">DOWNTIME OVERVIEW</div><h2>${escapeHtml(p.label)}</h2></div><div class="dashboard-overview-actions"><div class="dashboard-live-badge"><span></span>Downtime intelligence</div>${dashboardFilterButton()}</div></div>
  <div class="grid dashboard-kpis downtime-kpis dashboard-module-animate">
    <div class="card kpi-card downtime-kpi-total"><div class="kpi-top"><div class="kpi-label-pill">Total Downtime</div>${kpiGear('downtime_total')}</div><div class="kpi-body"><div><div class="metric kpi-value-downtime-total"${dashFitTitle(a.total,dashNum)} ${kpiStatusStyle('downtime_total',a.total)}>${dashFit(a.total,dashNum)}</div>${downtimeDeltaMarkup('downtime_total',a.total,prev.total,'Previous Period')}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('downtime_total',a.total)}>◷</span></div></div>
    <div class="card kpi-card downtime-kpi-unplanned"><div class="kpi-top"><div class="kpi-label-pill">Unplanned Downtime</div>${kpiGear('downtime_unplanned')}</div><div class="kpi-body"><div><div class="metric kpi-value-downtime-unplanned"${dashFitTitle(a.unplanned,dashNum)} ${kpiStatusStyle('downtime_unplanned',a.unplanned)}>${dashFit(a.unplanned,dashNum)}</div>${downtimeDeltaMarkup('downtime_unplanned',a.unplanned,prev.unplanned,'Previous Period')}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('downtime_unplanned',a.unplanned)}>!</span></div></div>
    <div class="card kpi-card downtime-kpi-planned"><div class="kpi-top"><div class="kpi-label-pill">Planned Downtime</div>${kpiGear('downtime_planned')}</div><div class="kpi-body"><div><div class="metric kpi-value-downtime-planned"${dashFitTitle(a.planned,dashNum)} ${kpiStatusStyle('downtime_planned',a.planned)}>${dashFit(a.planned,dashNum)}</div>${downtimeDeltaMarkup('downtime_planned',a.planned,prev.planned,'Previous Period')}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('downtime_planned',a.planned)}>◫</span></div></div>
    <div class="card kpi-card downtime-kpi-rate"><div class="kpi-top"><div class="kpi-label-pill">Downtime Rate</div>${kpiGear('downtime_rate')}</div><div class="kpi-body"><div><div class="metric kpi-value-downtime-rate" ${kpiStatusStyle('downtime_rate',a.downtimeRate)}>${dashPct2(a.downtimeRate)}</div>${downtimeDeltaMarkup('downtime_rate',a.downtimeRate,prev.downtimeRate,'Previous Period')}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('downtime_rate',a.downtimeRate)}>%</span></div></div>
    <div class="card kpi-card downtime-kpi-events"><div class="kpi-top"><div class="kpi-label-pill">Downtime Events</div>${kpiGear('downtime_events')}</div><div class="kpi-body"><div><div class="metric kpi-value-downtime-events"${dashFitTitle(a.events,dashNum)} ${kpiStatusStyle('downtime_events',a.events)}>${dashFit(a.events,dashNum)}</div>${downtimeDeltaMarkup('downtime_events',a.events,prev.events,'Previous Period')}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('downtime_events',a.events)}>◫</span></div></div>
  </div>
  ...
  dashboardStatus(`${a.d.prod.length.toLocaleString()} production captures · ${a.events.toLocaleString()} downtime events · ${dashNum(a.total)} minutes downtime.`);updateDashboardPeriodHint(p);renderDowntimeCharts(cmp);animateDashboardKpis();
}
...
function renderQualityDashboard(){
  ...
  if(!a.d.prod.length){box.innerHTML='<div class="panel dashboard-empty"><div class="eyebrow">QUALITY</div><h2>No Data</h2><p>No production captures match the selected Dashboard filters.</p><div class="dashboard-empty-actions">'+dashboardFilterButton()+'</div></div>';dashboardStatus('No Data in the selected filter scope.');return;}
  ...
  box.innerHTML=`${warning}<div class="dashboard-overview-head"><div><div class="eyebrow">QUALITY OVERVIEW</div><h2>${escapeHtml(p.label)}</h2></div><div class="dashboard-overview-actions"><div class="dashboard-live-badge"><span></span>Quality intelligence</div>${dashboardFilterButton()}</div></div>
  <div class="grid dashboard-kpis quality-kpis dashboard-module-animate">
    <div class="card kpi-card quality-kpi-scrap"><div class="kpi-top"><div class="kpi-label-pill">Scrap %</div>${kpiGear('quality_scrap_pct')}</div><div class="kpi-body"><div><div class="metric kpi-value-quality-scrap" ${kpiStatusStyle('quality_scrap_pct',a.scrapPct)}>${dashPct2(a.scrapPct)}</div>${qualityDeltaMarkup('quality_scrap_pct',a.scrapPct,prev.scrapPct,false,'Previous Period')}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('quality_scrap_pct',a.scrapPct)}>⚠</span></div></div>
    <div class="card kpi-card quality-kpi-ppm"><div class="kpi-top"><div class="kpi-label-pill">PPMs</div>${kpiGear('quality_ppm')}</div><div class="kpi-body"><div><div class="metric kpi-value-quality-ppm"${dashFitTitle(a.ppm,dashPpmText)} ${kpiStatusStyle('quality_ppm',a.ppm)}>${dashFit(a.ppm,dashPpmText)}</div>${qualityDeltaMarkup('quality_ppm',a.ppm,prev.ppm,false,'Previous Period')}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('quality_ppm',a.ppm)}>⦿</span></div></div>
    <div class="card kpi-card quality-kpi-scrap-qty"><div class="kpi-top"><div class="kpi-label-pill">Scrap Quantity</div>${kpiGear('quality_scrap_qty')}</div><div class="kpi-body"><div><div class="metric kpi-value-quality-scrap-qty"${dashFitTitle(a.scrap,dashNum)} ${kpiStatusStyle('quality_scrap_qty',a.scrap)}>${dashFit(a.scrap,dashNum)}</div>${qualityDeltaMarkup('quality_scrap_qty',a.scrap,prev.scrap,false,'Previous Period')}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('quality_scrap_qty',a.scrap)}>◈</span></div></div>
    <div class="card kpi-card quality-kpi-cost"><div class="kpi-top"><div class="kpi-label-pill">Scrap Cost</div>${kpiGear('quality_scrap_cost')}</div><div class="kpi-body"><div><div class="metric kpi-value-quality-cost"${dashFitTitle(a.scrapCost,dashMoney)} ${kpiStatusStyle('quality_scrap_cost',a.scrapCost)}>${dashFit(a.scrapCost,dashMoney)}</div>${qualityDeltaMarkup('quality_scrap_cost',a.scrapCost,prev.scrapCost,false,'Previous Period')}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('quality_scrap_cost',a.scrapCost)}>$</span></div></div>
    <div class="card kpi-card quality-kpi-copq"><div class="kpi-top"><div class="kpi-label-pill">COPQ %</div>${kpiGear('quality_copq')}</div><div class="kpi-body"><div><div class="metric kpi-value-quality-copq" ${kpiStatusStyle('quality_copq',a.copqPct)}>${dashPct2(a.copqPct)}</div>${qualityDeltaMarkup('quality_copq',a.copqPct,prev.copqPct,false,'Previous Period')}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('quality_copq',a.copqPct)}>◉</span></div></div>
    <div class="card kpi-card quality-kpi-yield"><div class="kpi-top"><div class="kpi-label-pill">Yield</div>${kpiGear('quality_yield')}</div><div class="kpi-body"><div><div class="metric kpi-value-quality-yield" ${kpiStatusStyle('quality_yield',a.yieldRatio)}>${dashPct2(a.yieldRatio)}</div>${qualityDeltaMarkup('quality_yield',a.yieldRatio,prev.yieldRatio,true,'Previous Period')}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('quality_yield',a.yieldRatio)}>✓</span></div></div>
  </div>
  ...
  dashboardStatus(`${a.d.prod.length.toLocaleString()} production captures · ${a.scrap.toLocaleString()} scrap pieces.`);updateDashboardPeriodHint(p);renderQualityCharts(cmp);bindDashboardConfig();animateDashboardKpis();
}
...
function renderDashboardGeneral(){if(dashboardState.tab==='Plant Now')return renderPlantNowDashboard();const box=document.getElementById('dashboardGeneral');if(!box)return;const cmp=dashboardComparison(),a=cmp.current,prev=cmp.previous,p=cmp.period;if(!a.d.prod.length){box.innerHTML='<div class="panel dashboard-empty"><div class="eyebrow">GENERAL</div><h2>No Data</h2><p>No production captures match the selected Dashboard filters.</p><div class="dashboard-empty-actions">'+dashboardFilterButton()+'</div></div>';dashboardStatus('No Data in the selected filter scope.');return;}
 ...
 box.innerHTML=`${warning}<div class="dashboard-overview-head"><div><div class="eyebrow">GENERAL OVERVIEW</div><h2>${escapeHtml(p.label)}</h2></div><div class="dashboard-overview-actions"><div class="dashboard-live-badge"><span></span>Live operational view</div>${dashboardFilterButton()}</div></div>
 <div class="grid dashboard-kpis">
 <div class="card kpi-card kpi-oee"><div class="kpi-top"><div class="kpi-label-pill">OEE</div>${kpiGear('oee')}</div><div class="kpi-body"><div><div class="metric kpi-value-oee" ${kpiStatusStyle('oee',a.oee)}>${dashPct2(a.oee)}</div>${kpiDeltaMarkup('oee',a.oee,prev.oee,true,priorLabel)}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('oee',a.oee)}>◔</span></div></div>
 <div class="card kpi-card kpi-production"><div class="kpi-top"><div class="kpi-label-pill">Production</div>${kpiGear('production')}</div><div class="kpi-body"><div><div class="metric kpi-value-production" ${kpiStatusStyle('production',a.production)}>${dashNum(a.production)}</div>${kpiDeltaMarkup('production',a.production,prev.production,true,priorLabel)}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('production',a.production)}>▤</span></div></div>
 <div class="card kpi-card kpi-scrap"><div class="kpi-top"><div class="kpi-label-pill">Scrap</div>${kpiGear('scrap')}</div><div class="kpi-body"><div><div class="metric kpi-value-scrap" ${kpiStatusStyle('scrap',a.scrapPct)}>${dashPct2(a.scrapPct)}</div>${kpiDeltaMarkup('scrap',a.scrapPct,prev.scrapPct,false,priorLabel)}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('scrap',a.scrapPct)}>⚠</span></div></div>
 <div class="card kpi-card kpi-ppm"><div class="kpi-top"><div class="kpi-label-pill">PPMs</div>${kpiGear('ppm')}</div><div class="kpi-body"><div><div class="metric kpi-value-ppm" ${kpiStatusStyle('ppm',a.ppm)}>${a.ppm==null?'N/A':Math.round(a.ppm).toLocaleString()}</div>${kpiDeltaMarkup('ppm',a.ppm,prev.ppm,false,priorLabel)}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('ppm',a.ppm)}>⦿</span></div></div>
 <div class="card kpi-card kpi-yield"><div class="kpi-top"><div class="kpi-label-pill">Yield</div>${kpiGear('yield')}</div><div class="kpi-body"><div><div class="metric kpi-value-yield" ${kpiStatusStyle('yield',a.yieldRatio)}>${dashPct2(a.yieldRatio)}</div>${kpiDeltaMarkup('yield',a.yieldRatio,prev.yieldRatio,true,priorLabel)}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('yield',a.yieldRatio)}>✓</span></div></div>
 <div class="card kpi-card kpi-copq"><div class="kpi-top"><div class="kpi-label-pill">COPQ</div>${kpiGear('copq')}</div><div class="kpi-body"><div><div class="metric kpi-value-copq" ${kpiStatusStyle('copq',a.copqPct)}>${dashPct2(a.copqPct)}</div>${kpiDeltaMarkup('copq',a.copqPct,prev.copqPct,false,priorLabel)}</div><span class="kpi-connector" aria-hidden="true"></span><span class="kpi-icon" aria-hidden="true" ${kpiStatusStyle('copq',a.copqPct)}>◈</span></div></div>
 </div>
 <div class="dashboard-chart-grid dashboard-chart-grid-six">
 <div class="panel chart-panel"><div class="chart-head"><div><h2>OEE</h2><p>Daily OEE.</p></div>${chartGear('oee')}<span class="chart-chip">%</span></div><div class="chart-wrap"><canvas id="chartOee"></canvas></div><div class="chart-reference-note-host" data-chart-note="oee"></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>Production</h2><p>Daily pieces produced.</p></div>${chartGear('production')}<span class="chart-chip">Pieces</span></div><div class="chart-wrap"><canvas id="chartProduction"></canvas></div><div class="chart-reference-note-host" data-chart-note="production"></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>Scrap %</h2><p>Average scrap rate by part number.</p></div>${chartGear('scrap')}<span class="chart-chip">%</span></div><div class="chart-wrap"><canvas id="chartScrap"></canvas></div><div class="chart-reference-note-host" data-chart-note="scrap"></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>PPMs</h2><p>Scrap pieces per million produced.</p></div>${chartGear('ppm')}<span class="chart-chip">PPM</span></div><div class="chart-wrap"><canvas id="chartPpm"></canvas></div><div class="chart-reference-note-host" data-chart-note="ppm"></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>Yield</h2><p>Good pieces / production.</p></div>${chartGear('yield')}<span class="chart-chip">%</span></div><div class="chart-wrap"><canvas id="chartYield"></canvas></div><div class="chart-reference-note-host" data-chart-note="yield"></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>COPQ %</h2><p>Poor quality cost / total produced cost.</p></div>${chartGear('copq')}<span class="chart-chip">%</span></div><div class="chart-wrap"><canvas id="chartCopq"></canvas></div><div class="chart-reference-note-host" data-chart-note="copq"></div></div>
 </div>`;
...
function chartReferenceMarkup(key){const c=dashboardPrefs().charts?.[key]||{},m=DASH_CHART_META[key],fmt=v=>v==null?'':(m.unit==='%'?`${(v*100).toLocaleString(undefined,{maximumFractionDigits:2})}%`:Number(v).toLocaleString(undefined,{maximumFractionDigits:2})),items=[['Min',c.min],['Objetivo',c.target],['Max',c.max]].filter(([,v])=>v!=null);return items.length?`<div class="chart-reference-note">${items.map(([l,v])=>`<span><b>${l}</b> ${fmt(v)}</span>`).join('')}</div>`:'';}
const HALF_RING_PLUGIN={id:'guvelHalfRingCenter',afterDraw(chart,args,opts){if(!opts||opts.value==null)return;const {ctx,chartArea}=chart;if(!chartArea)return;const x=(chartArea.left+chartArea.right)/2,y=chartArea.bottom-8;ctx.save();ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillStyle=opts.color||'#14202b';ctx.font='900 25px Arial';ctx.fillText(opts.text||'',x,y-20);ctx.restore();}};
function renderDashboardCharts(cmp){if(!window.Chart){dashboardStatus('Chart library unavailable.','error');return;}const labels=niceLabels(cmp.current.daily),daily=labels.map(day=>dashboardDailyMetric(day)),prod=daily.map(x=>x.production),oee=daily.map(x=>x.oee==null?null:x.oee*100),yieldVals=daily.map(x=>x.yieldRatio==null?null:x.yieldRatio*100),ppmVals=daily.map(x=>x.ppm==null?null:x.ppm),scrapVals=daily.map(x=>x.scrapPct==null?null:x.scrapPct*100),copqVals=daily.map(x=>x.copqPct==null?null:x.copqPct*100);const refs=k=>chartReferenceDatasets(k,labels);const yOpts=(key,extra={})=>{const c=dashboardPrefs().charts?.[key]||{},pct=DASH_CHART_META[key].unit==='%',o={...extra};if(c.min!=null)o.min=pct?c.min*100:c.min;if(c.max!=null)o.max=pct?c.max*100:c.max;return o;};const isSingleDay=cmp.period.period==='Today'||cmp.period.period==='Previous Day';const half=(key,id,label,value,pct)=>{const c=dashboardPrefs().charts?.[key]||{},raw=value==null?0:(pct?value*100:value),safe=Math.max(0,Number(raw)||0),scale=pct?100:(Number(c.max)||Number(c.target)||safe||1),filled=Math.min(scale,safe),data=[filled,Math.max(0,scale-filled)],text=value==null?'N/A':(pct?`${safe.toLocaleString(undefined,{minimumFractionDigits:key==='oee'||key==='scrap'||key==='yield'||key==='copq'?2:1,maximumFractionDigits:key==='oee'||key==='scrap'||key==='yield'||key==='copq'?2:1})}%`:Number(value).toLocaleString(undefined,{maximumFractionDigits:2}));chartCreate(key,document.getElementById(id),{type:'doughnut',data:{labels:[label,'Remaining'],datasets:[{data,backgroundColor:['#0cc0df','rgba(12,192,223,.12)'],borderColor:['#0cc0df','rgba(12,192,223,.12)'],borderWidth:1}]},options:{responsive:true,maintainAspectRatio:false,rotation:270,circumference:180,cutout:'72%',plugins:{legend:{display:false},tooltip:{enabled:false},guvelHalfRingCenter:{value,color:'#14202b',text}}},plugins:[HALF_RING_PLUGIN]});};const line=(key,id,label,data,extra={})=>chartCreate(key,document.getElementById(id),{type:'line',data:{labels,datasets:[{label,data,borderWidth:3,pointRadius:4,pointHoverRadius:7,tension:.3,fill:true,backgroundColor:key==='oee'||key==='yield'?'rgba(12,192,223,.12)':'rgba(255,49,49,.10)',borderColor:key==='oee'||key==='yield'?'#0cc0df':'#ff3131',pointBackgroundColor:'#fff',pointBorderWidth:2},...refs(key)]},options:chartBase({y:yOpts(key,extra)})});if(isSingleDay){half('oee','chartOee','OEE',cmp.current.oee,true);half('production','chartProduction','Production',cmp.current.production,false);half('scrap','chartScrap','Scrap',cmp.current.scrapPct,true);half('ppm','chartPpm','PPMs',cmp.current.ppm,false);half('yield','chartYield','Yield',cmp.current.yieldRatio,true);half('copq','chartCopq','COPQ',cmp.current.copqPct,true);}else{line('oee','chartOee','OEE',oee,{beginAtZero:true,ticks:{callback:v=>v+'%'}});chartCreate('production',document.getElementById('chartProduction'),{type:'bar',data:{labels,datasets:[{label:'Production',data:prod,borderRadius:3,backgroundColor:'rgba(12,192,223,.72)',borderColor:'#0cc0df',borderWidth:1},...refs('production')]},options:chartBase({y:yOpts('production')})});line('scrap','chartScrap','Scrap %',scrapVals,{beginAtZero:true,ticks:{callback:v=>v+'%'}});line('ppm','chartPpm','PPM',ppmVals,{});line('yield','chartYield','Yield',yieldVals,{min:80,max:100,ticks:{callback:v=>v+'%'}});line('copq','chartCopq','COPQ %',copqVals,{beginAtZero:true,ticks:{callback:v=>v+'%'}});}document.querySelectorAll('[data-chart-note]').forEach(el=>{el.innerHTML=chartReferenceMarkup(el.dataset.chartNote);});bindDashboardConfig();[['oee',cmp.current.oee,'.kpi-value-oee'],['production',cmp.current.production,'.kpi-value-production'],['scrap',cmp.current.scrapPct,'.kpi-value-scrap'],['ppm',cmp.current.ppm,'.kpi-value-ppm'],['yield',cmp.current.yieldRatio,'.kpi-value-yield'],['copq',cmp.current.copqPct,'.kpi-value-copq']].forEach(([k,v,sel])=>document.querySelectorAll(sel).forEach(e=>{const c=kpiColorFor(k,v);e.style.color=c||'';}));}
...
async function renderStatusFoundation(){
  ...
  const modal=document.getElementById('statusMachineModal');
  const renderCards=()=>{
    ...
    grid.querySelectorAll('[data-machine-id]').forEach(b=>b.addEventListener('click',()=>openMachineProfile(b.dataset.machineId)));
  };
...
function dashboardActiveFilterCount(){const f=dashboardState.filters||{};let n=['customer','part','shift','machine'].filter(k=>f[k]).length;if((f.period||'This Month')!=='This Month')n++;return n;}
function dashboardFilterButton(){const n=dashboardActiveFilterCount();return `<button type="button" class="dashboard-filter-btn${n?' has-filters':''}" data-dashboard-filter-toggle aria-haspopup="dialog" aria-expanded="false" aria-controls="dashboardFilters"><svg viewBox="0 0 16 16" width="14" height="14" aria-hidden="true"><path d="M1.5 2.5h13l-5 6v4.5l-3 1.5v-6z" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/></svg><span>Filters</span>${n?`<b aria-label="${n} active">${n}</b>`:''}</button>`;}
(()=>{
  let lastToggle=null;
  const pop=()=>document.getElementById('dashboardFilters'),back=()=>document.getElementById('dashboardFilterBackdrop');
  const anchor=()=>{const all=[...document.querySelectorAll('[data-dashboard-filter-toggle]')].filter(b=>b.offsetParent!==null);return all[0]||null;};
  const isSheet=()=>window.matchMedia('(max-width: 720px)').matches;
  const place=()=>{const p=pop();if(!p||p.hidden)return;const b=anchor();document.querySelectorAll('[data-dashboard-filter-toggle]').forEach(x=>x.setAttribute('aria-expanded',String(x===b)));const bd=back();if(bd)bd.hidden=!isSheet();if(isSheet()||!b){p.classList.add('is-sheet');p.style.top=p.style.right=p.style.maxHeight='';return;}p.classList.remove('is-sheet');const r=b.getBoundingClientRect(),gap=8;let top=r.bottom+gap;const h=p.offsetHeight;if(top+h>window.innerHeight-12&&r.top-gap-h>12)top=r.top-gap-h;p.style.top=Math.max(12,top)+'px';p.style.right=Math.max(12,window.innerWidth-r.right)+'px';p.style.maxHeight=(window.innerHeight-Math.max(12,top)-12)+'px';};
  const open=(btn)=>{const p=pop();if(!p)return;lastToggle=btn||anchor();p.hidden=false;place();requestAnimationFrame(place);(p.querySelector('#dashPeriod')||p).focus({preventScroll:true});};
  const close=(returnFocus=true)=>{const p=pop();if(!p||p.hidden)return;p.hidden=true;const bd=back();if(bd)bd.hidden=true;document.querySelectorAll('[data-dashboard-filter-toggle]').forEach(x=>x.setAttribute('aria-expanded','false'));if(returnFocus){const b=anchor()||lastToggle;b?.focus({preventScroll:true});}};
  window.dashboardFiltersRefresh=()=>{requestAnimationFrame(place);};
  document.addEventListener('click',e=>{const t=e.target;const tog=t.closest?.('[data-dashboard-filter-toggle]');if(tog){const p=pop();if(p&&!p.hidden)close(false);else open(tog);return;}if(t.closest?.('#dashboardFiltersClose,#dashboardFiltersDone')){close();return;}const p=pop();if(p&&!p.hidden&&!t.closest?.('#dashboardFilters'))close(false);});
...
/* =====================================================================
   Phase 3.0.C — Plant Now dashboard
   Status comes from live production sessions (RUNNING = running, none = idle).
   The percentage is each machine's OEE in the selected Dashboard period and filters.
   ===================================================================== */
const PLANT_DEFAULT_TARGETS={oee:.85,availability:.90,performance:.95,quality:.99};
let plantLiveTimer=null;
function plantTargets(){const p=dashboardPrefs();return Object.assign({},PLANT_DEFAULT_TARGETS,p.plantNow||{});}
function plantMachineMetrics(range){const saved=dashboardState.filters.machine,out=new Map();try{for(const m of dashboardState.machines||[]){dashboardState.filters.machine=m.id;const d=prepareOeeData(dashboardFiltered(range)),mm=buildOeeMetrics(d);out.set(m.id,{oee:mm.oee,captures:d.prod.length});}}finally{dashboardState.filters.machine=saved;}return out;}
function plantFamily(code){const m=/^[A-Za-z]+/.exec(String(code||''));return m?m[0].toUpperCase():'#';}
function plantPct(v,d=1){return v==null||!isFinite(v)?'—':`${(v*100).toFixed(d)}%`;}
function plantMachineModel(){
  const cmp=dashboardComparison(),t=plantTargets(),per=plantMachineMetrics(cmp.currentRange);
  const running=new Map((dashboardState.sessions||[]).filter(x=>String(x.status||'').toUpperCase()==='RUNNING').map(x=>[x.machine_id,x]));
  const machines=[...(dashboardState.machines||[])].sort((a,b)=>String(a.code||'').localeCompare(String(b.code||''),undefined,{numeric:true}));
  const list=machines.map(m=>{const s=running.get(m.id),k=per.get(m.id)||{},oee=k.oee;const state=s?(oee!=null&&oee<t.oee?'watch':'run'):'idle';return {m,s,oee,captures:k.captures||0,state};});
  return {cmp,t,list};
}
function plantHexMarkup(x){
  const label=x.state==='idle'?'Idle':plantPct(x.oee,0);
  const parts=[`${x.m.code}${x.m.name?' — '+x.m.name:''}`,x.state==='idle'?'Idle, no active session':x.state==='watch'?'Running, below OEE target':'Running'];
  if(x.s){const pn=x.s.part_numbers?.part_number,op=x.s.operations;if(pn)parts.push(`Part ${pn}`);if(op)parts.push(`Op ${op.operation_number||''} ${op.operation_name||''}`.trim());if(x.s.lot_number)parts.push(`Lot ${x.s.lot_number}`);}
  parts.push(`OEE in period: ${plantPct(x.oee)}`);
  const tip=escapeHtml(parts.join('\n'));
  const pnTxt=x.s?.part_numbers?.part_number||'',op=x.s?.operations,opTxt=op&&op.operation_number!=null&&op.operation_number!==''?`Op ${op.operation_number}`:'',opName=op?.operation_name||'';
  const meta=x.s&&(pnTxt||opTxt||opName)?`<span class="ph-meta">${pnTxt?`<em class="ph-pn">${escapeHtml(pnTxt)}</em>`:''}${opTxt?`<em class="ph-op">${escapeHtml(opTxt)}</em>`:''}${opName?`<em class="ph-opname">${escapeHtml(opName)}</em>`:''}</span>`:'';
  return `<button type="button" class="plant-hex is-${x.state}" data-plant-machine="${escapeHtml(x.m.id)}" title="${tip}" aria-label="${escapeHtml(parts.join('. '))}"><i aria-hidden="true"></i><span><small>${escapeHtml(x.m.code||'—')}</small><b>${escapeHtml(label)}</b>${meta}</span></button>`;
}
function plantLayoutHive(host,list){
  if(!host)return;const w=host.clientWidth||600,gap=6;const famSizes={};for(const x of list){const f=plantFamily(x.m.code);famSizes[f]=(famSizes[f]||0)+1;}const widest=Math.min(w<520?2:w<760?3:99,Math.max(2,...Object.values(famSizes)));const fit=Math.floor((w-(widest-1)*gap-gap/2)/(widest+.5));const hexW=Math.max(w<520?96:104,Math.min(w<520?132:140,fit));
  host.style.setProperty('--hex-w',hexW+'px');
  const perRow=Math.max(2,Math.floor((w-(hexW+gap)/2)/(hexW+gap)));
  const fams=[];for(const x of list){const f=plantFamily(x.m.code);let g=fams.find(z=>z.f===f);if(!g){g={f,items:[]};fams.push(g);}g.items.push(x);}
  let rowIndex=0,html='';
  for(const g of fams){for(let i=0;i<g.items.length;i+=perRow){const row=g.items.slice(i,i+perRow);html+=`<div class="plant-row${rowIndex%2?' is-offset':''}${rowIndex===0?' is-first':''}">${row.map(plantHexMarkup).join('')}</div>`;rowIndex++;}}
  host.innerHTML=html||'<div class="dashboard-mini-empty">No machines registered.</div>';
}
function plantBar(label,v,target){const pct=v==null?0:Math.max(0,Math.min(1,v))*100,low=v!=null&&v<target;return `<div class="plant-bar"><div class="plant-bar-top"><span>${label}</span><span class="num${low?' is-low':''}">${plantPct(v)}</span></div><div class="plant-track"><div class="plant-fill${low?' is-low':''}" style="width:${pct}%"></div><div class="plant-tick" style="left:${Math.min(100,target*100)}%" title="Target ${plantPct(target,0)}"></div></div></div>`;}
function renderPlantNowDashboard(){
  const box=document.getElementById('dashboardPlantNow');if(!box)return;
  const {cmp,t,list}=plantMachineModel(),a=cmp.current,p=cmp.period;
  const nRun=list.filter(x=>x.state!=='idle').length,nWatch=list.filter(x=>x.state==='watch').length,nIdle=list.filter(x=>x.state==='idle').length;
  const gapPts=a.oee==null?null:(a.oee-t.oee)*100;
  const targetLine=gapPts==null?'No OEE data in this period':`Target ${plantPct(t.oee,0)}, ${Math.abs(gapPts).toFixed(1)} pts ${gapPts<0?'below':'above'}`;
  box.innerHTML=`<div class="dashboard-overview-head"><div><div class="eyebrow">PLANT NOW</div><h2>${escapeHtml(p.label)}</h2></div><div class="dashboard-overview-actions"><div class="dashboard-live-badge"><span></span>Live machine status</div>${dashboardFilterButton()}</div></div>
  <div class="plant-grid">
    <section class="panel plant-hive-panel" aria-labelledby="plantHiveTitle">
      <div class="plant-head"><h2 id="plantHiveTitle">Plant now</h2><span class="label">${list.length} machine${list.length===1?'':'s'} · ${nRun} running${nWatch?` · ${nWatch} below target`:''} · ${nIdle} idle</span></div>
      <div class="plant-hive" id="plantHive"></div>
      <div class="plant-legend"><span><i class="is-run"></i>Running</span><span><i class="is-watch"></i>Below target (OEE &lt; ${plantPct(t.oee,0)})</span><span><i class="is-idle"></i>Idle</span></div>
    </section>
    <section class="panel plant-oee-panel" aria-labelledby="plantOeeTitle">
      <div class="plant-head"><h2 id="plantOeeTitle">OEE</h2><button type="button" class="link-btn" id="plantTargetsBtn">Targets</button></div>
      <div class="plant-oee-big num">${a.oee==null?'—':(a.oee*100).toFixed(1)}<small>${a.oee==null?'':'%'}</small></div>
      <div class="plant-oee-target${gapPts!=null&&gapPts<0?' is-low':''}">${targetLine}</div>
      <div class="plant-bars">${plantBar('Availability',a.availability,t.availability)}${plantBar('Performance',a.performance,t.performance)}${plantBar('Quality',a.quality,t.quality)}</div>
      <p class="plant-note">Percentages use the selected period and filters. Machine status updates every minute.</p>
    </section>
  </div>`;
  const hive=document.getElementById('plantHive');plantLayoutHive(hive,list);
  if(window.ResizeObserver){let lastW=hive.clientWidth;const ro=new ResizeObserver(()=>{if(!hive.isConnected){ro.disconnect();return;}if(Math.abs(hive.clientWidth-lastW)>4){lastW=hive.clientWidth;plantLayoutHive(hive,list);}});ro.observe(hive);}
  hive.addEventListener('click',e=>{const b=e.target.closest('[data-plant-machine]');if(b)plantOpenMachine(b.dataset.plantMachine);});
  document.getElementById('plantTargetsBtn')?.addEventListener('click',plantOpenTargets);
  dashboardStatus(`${a.d.prod.length} production captures in period · ${nRun} of ${list.length} machines running.`);
  plantStartLive();
}
function plantOpenMachine(id){const nav=document.querySelector('#nav button[data-page="Status"]');if(!nav)return;nav.click();const t0=Date.now();const tick=()=>{const card=document.querySelector(`.status-machine-card-button[data-machine-id="${CSS.escape(id)}"]`);if(card){card.click();return;}if(Date.now()-t0<5000)setTimeout(tick,120);};setTimeout(tick,120);}
function plantStartLive(){if(plantLiveTimer)return;plantLiveTimer=setInterval(async()=>{const active=typeof current!=='undefined'&&current==='Dashboard'&&dashboardState.tab==='Plant Now'&&document.getElementById('dashboardPlantNow');if(!active){clearInterval(plantLiveTimer);plantLiveTimer=null;return;}if(document.hidden)return;if(document.getElementById('dashboardFilters')?.hidden===false)return;try{const r=await sb.from('machine_production_sessions').select('machine_id,status,started_at,lot_number,part_numbers(part_number),operations(operation_number,operation_name)').eq('company_id',activeCompanyId);if(!r.error){dashboardState.sessions=r.data||[];renderPlantNowDashboard();}}catch(e){console.warn('Plant Now refresh',e);}},60000);}
function plantOpenTargets(){
  ...
  document.body.appendChild(ov);const done=()=>{ov.remove();document.getElementById('plantTargetsBtn')?.focus();};
  ov.addEventListener('click',e=>{if(e.target===ov||e.target.closest('[data-close]'))done();});
  ov.addEventListener('keydown',e=>{if(e.key==='Escape')done();});
  ov.querySelector('[data-reset]').onclick=()=>{const pr=dashboardPrefs();delete pr.plantNow;saveDashboardPrefs(pr);done();renderPlantNowDashboard();};
  ov.querySelector('[data-save]').onclick=()=>{const pr=dashboardPrefs(),next={};ov.querySelectorAll('[data-target]').forEach(i=>{const v=Number(i.value);if(isFinite(v)&&v>=0&&v<=100)next[i.dataset.target]=v/100;});pr.plantNow=Object.assign({},plantTargets(),next);saveDashboardPrefs(pr);done();renderPlantNowDashboard();};
