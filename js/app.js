...
function productionGaugeMarkup(a){
  const vals=[a.availability,a.performance,a.quality].map(v=>v==null?0:Math.max(0,Math.min(1,v)));
  const colors=['#0cc0df','#16a957','#ff3131'],labels=['Availability','Performance','Quality'];
  const rings=[{r:118,w:18},{r:91,w:18},{r:64,w:18}];
  const paths=rings.map((ring,i)=>{const c=2*Math.PI*ring.r;const progress=c*vals[i];return `<circle class="oee-ring-track" cx="160" cy="160" r="${ring.r}" stroke-width="${ring.w}"/><circle class="oee-ring-value" cx="160" cy="160" r="${ring.r}" stroke="${colors[i]}" stroke-width="${ring.w}" stroke-dasharray="${progress.toFixed(2)} ${(c-progress).toFixed(2)}"/>`;}).join('');
  return `<div class="production-gauge-card oee-premium"><div class="production-gauge-title-row"><div><div class="production-gauge-title">OEE</div><div class="gauge-subtitle">Overall Equipment Effectiveness</div></div>${kpiGear('prod_oee')}</div><div class="oee-ring-wrap full premium"><svg class="oee-ring" viewBox="0 0 320 320" role="img" aria-label="OEE ${gaugeValue(a.oee)}"><g transform="rotate(-90 160 160)">${paths}</g><circle cx="160" cy="160" r="39" class="oee-center-disc"/><circle cx="160" cy="160" r="3" class="oee-center-dot"/></svg><div class="oee-ring-center"><strong>${gaugeValue(a.oee)}</strong><span>OEE PERCENTAGE</span></div></div><div class="gauge-legend premium">${labels.map((l,i)=>`<span><i class="gauge-dot" style="background:${colors[i]}"></i><em>${l}</em><b>${gaugeValue([a.availability,a.performance,a.quality][i])}</b></span>`).join('')}</div></div>`;
}
function planVsActualMarkup(a){
  ...
  const achievement=planned>0?Math.max(0,actual/planned*100):0,clamped=Math.min(100,achievement);
  ...
  const ticks=Array.from({length:11},(_,i)=>{const d=180+i*18,outer=143,inner=i%5===0?128:134,a=polar(d,outer),b=polar(d,inner);return `<line x1="${a[0]}" y1="${a[1]}" x2="${b[0]}" y2="${b[1]}"/>`;}).join('');
  const value=planned?achievement.toFixed(1)+'%':'—';
  return `<div class="production-plan-card odometer-premium"><div class="production-gauge-title-row"><div><div class="production-gauge-title">Plan vs Actual</div><div class="gauge-subtitle">Production achievement</div></div>${kpiGear('plan_actual')}</div><div class="odometer clean-odometer guvel-odometer"><svg viewBox="0 0 360 245" role="img" aria-label="Plan vs Actual ${planned?achievement.toFixed(1):'N/A'} percent"><defs><linearGradient id="guvelNeedle" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#143980"/><stop offset="100%" stop-color="#0cc0df"/></linearGradient></defs><path d="${arc(180,360,132)}" class="odo-track"/><path d="${arc(180,306,132)}" class="odo-zone odo-green"/><path d="${arc(306,324,132)}" class="odo-zone odo-yellow"/><path d="${arc(324,360,132)}" class="odo-zone odo-red"/><g class="odo-ticks">${ticks}</g><g class="odo-needle" style="transform:rotate(${angle-270}deg)"><line x1="180" y1="190" x2="180" y2="76"/><circle cx="180" cy="190" r="13"/></g><circle cx="180" cy="190" r="7" class="odo-hub"/><text x="180" y="151" text-anchor="middle" class="odo-value">${value}</text><text x="180" y="166" text-anchor="middle" class="odo-label">ACHIEVEMENT</text><text x="42" y="222" text-anchor="middle" class="odo-scale">0%</text><text x="180" y="43" text-anchor="middle" class="odo-scale">70%</text><text x="318" y="222" text-anchor="middle" class="odo-scale">100%</text></svg></div><div class="plan-actual-values premium"><div><span>Planned</span><strong>${planned?Math.round(planned).toLocaleString():'N/A'}</strong></div><div><span>Actual</span><strong>${actual.toLocaleString()}</strong></div><div><span>Achievement</span><strong>${planned?achievement.toFixed(1)+'%':'N/A'}</strong></div></div><small class="gauge-note">Planned downtime is excluded from available production time.</small></div>`;
}
