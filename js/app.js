...
function productionGaugeMarkup(a){
  const vals=[a.availability,a.performance,a.quality].map(v=>v==null?0:Math.max(0,Math.min(1,v))),colors=['#0cc0df','#16a957','#ff3131'],labels=['Availability','Performance','Quality'];
  const rings=[{r:118,w:20},{r:88,w:18},{r:58,w:16}];
  const paths=rings.map((ring,i)=>{const c=2*Math.PI*ring.r;return `<circle class="oee-ring-track" cx="160" cy="160" r="${ring.r}" stroke-width="${ring.w}"/><circle class="oee-ring-value" cx="160" cy="160" r="${ring.r}" stroke="${colors[i]}" stroke-width="${ring.w}" stroke-dasharray="${c}" stroke-dashoffset="${c*(1-vals[i])}"/>`;}).join('');
  return `<div class="production-gauge-card oee-premium"><div class="production-gauge-title-row"><div><div class="production-gauge-title">OEE</div><div class="gauge-subtitle">Overall Equipment Effectiveness</div></div>${kpiGear('prod_oee')}</div><div class="oee-ring-wrap full premium"><svg class="oee-ring" viewBox="0 0 320 320" role="img" aria-label="OEE ${gaugeValue(a.oee)}"><defs><filter id="oeeGlow"><feGaussianBlur stdDeviation="2.2" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs><g transform="rotate(-90 160 160)">${paths}</g><circle cx="160" cy="160" r="30" fill="none" stroke="#edf3f5" stroke-width="1.5"/><circle cx="160" cy="160" r="4" fill="#0cc0df"/></svg><div class="oee-ring-center"><strong>${gaugeValue(a.oee)}</strong><span>OEE SCORE</span></div></div><div class="gauge-legend premium">${labels.map((l,i)=>`<span><i class="gauge-dot" style="background:${colors[i]}"></i><em>${l}</em><b>${gaugeValue([a.availability,a.performance,a.quality][i])}</b></span>`).join('')}</div></div>`;
}
function planVsActualMarkup(a){
  let planned=0,actual=a.production||0;
  for(const r of a.d.prod){const q=Number(r.production_quantity||0),sh=r.shifts,dur=shiftDurationSeconds(sh),excluded=Number(sh?.excluded_planned_minutes||0)*60,available=dur==null?0:Math.max(0,dur-excluded),key=`${r.part_number_id||'unknown-part'}|${r.operation_id||'unknown-operation'}|${r.machine_id||'unknown-machine'}`,cfg=dashboardState.cycleTimes.find(x=>x.key===key),cy=Number(cfg?.cycle_time_seconds||r.operations?.ideal_cycle_time_seconds||0);if(available>0&&cy>0)planned+=available/cy;}
  const achievement=planned>0?Math.max(0,actual/planned*100):0,clamped=Math.min(100,achievement),angle=-90+clamped*1.8;
  const zone=v=>v<70?'#ff3131':v<80?'#f0b323':'#16a957';
  const ticks=Array.from({length:21},(_,i)=>{const a=(-90+i*9)*Math.PI/180,r1=112,r2=i%5===0?99:105,cx=160,cy=160;return `<line x1="${cx+Math.cos(a)*r1}" y1="${cy+Math.sin(a)*r1}" x2="${cx+Math.cos(a)*r2}" y2="${cy+Math.sin(a)*r2}"/>`;}).join('');
  return `<div class="production-plan-card odometer-premium"><div class="production-gauge-title-row"><div><div class="production-gauge-title">Plan vs Actual</div><div class="gauge-subtitle">Production achievement</div></div>${kpiGear('plan_actual')}</div><div class="odometer reference premium"><svg viewBox="0 0 320 190" role="img" aria-label="Plan vs Actual ${planned?achievement.toFixed(1):'N/A'} percent"><defs><linearGradient id="odoGreen" x1="0" x2="1"><stop offset="0%" stop-color="#16a957"/><stop offset="100%" stop-color="#0b8744"/></linearGradient></defs><path d="M 35 160 A 125 125 0 0 1 285 160" fill="none" stroke="#e8eef1" stroke-width="27" stroke-linecap="butt"/><path d="M 35 160 A 125 125 0 0 1 285 160" fill="none" stroke="#ff3131" stroke-width="25" pathLength="100" stroke-dasharray="70 30"/><path d="M 35 160 A 125 125 0 0 1 285 160" fill="none" stroke="#f0b323" stroke-width="25" pathLength="100" stroke-dasharray="10 90" stroke-dashoffset="-70"/><path d="M 35 160 A 125 125 0 0 1 285 160" fill="none" stroke="url(#odoGreen)" stroke-width="25" pathLength="100" stroke-dasharray="20 80" stroke-dashoffset="-80"/><g class="odometer-tick-lines">${ticks}</g><g class="odometer-needle" style="transform:rotate(${angle}deg)"><line x1="160" y1="160" x2="160" y2="52"/><circle cx="160" cy="160" r="12"/></g><circle cx="160" cy="160" r="5" fill="#fff"/><text x="160" y="126" text-anchor="middle" class="odometer-value">${planned?achievement.toFixed(1)+'%':'—'}</text><text x="160" y="143" text-anchor="middle" class="odometer-label">ACHIEVEMENT</text><text x="35" y="181" text-anchor="middle" class="odometer-scale-label">0%</text><text x="247" y="181" text-anchor="middle" class="odometer-scale-label">70%</text><text x="285" y="181" text-anchor="middle" class="odometer-scale-label">100%</text></svg></div><div class="plan-actual-values premium"><div><span>Planned</span><strong>${planned?Math.round(planned).toLocaleString():'N/A'}</strong></div><div><span>Actual</span><strong>${actual.toLocaleString()}</strong></div><div><span>Achievement</span><strong>${planned?achievement.toFixed(1)+'%':'N/A'}</strong></div></div><small class="gauge-note">Planned downtime is excluded from available production time.</small></div>`;
}
function planVsActualMarkup(a){
  let planned=0,actual=a.production||0;
  for(const r of a.d.prod){const q=Number(r.production_quantity||0),sh=r.shifts,dur=shiftDurationSeconds(sh),excluded=Number(sh?.excluded_planned_minutes||0)*60,available=dur==null?0:Math.max(0,dur-excluded),key=`${r.part_number_id||'unknown-part'}|${r.operation_id||'unknown-operation'}|${r.machine_id||'unknown-machine'}`,cfg=dashboardState.cycleTimes.find(x=>x.key===key),cy=Number(cfg?.cycle_time_seconds||r.operations?.ideal_cycle_time_seconds||0);if(available>0&&cy>0)planned+=available/cy;}
  const achievement=planned>0?Math.max(0,actual/planned*100):0,clamped=Math.min(100,achievement),angle=-90+clamped*1.8;
  const zone=(v)=>v<70?'#ff3131':v<80?'#f0b323':'#16a957';
  return `<div class="production-plan-card"><div class="production-gauge-title-row"><div class="production-gauge-title">Plan vs Actual</div>${kpiGear('plan_actual')}</div><div class="odometer reference"><div class="odometer-zone red"></div><div class="odometer-zone yellow"></div><div class="odometer-zone green"></div><div class="odometer-track"></div><div class="odometer-ticks"></div><div class="odometer-pointer" style="--pointer-color:${zone(achievement)};transform:rotate(${angle}deg)"></div><div class="odometer-hub"></div><div class="odometer-center"><strong>${planned?`${achievement.toFixed(1)}%`:'—'}</strong><span>Achievement</span></div><div class="odometer-scale"><span>0%</span><span>70%</span><span>80%</span><span>100%</span></div></div><div class="plan-actual-values"><div><span>Planned</span><strong>${planned?Math.round(planned).toLocaleString():'N/A'}</strong></div><div><span>Actual</span><strong>${actual.toLocaleString()}</strong></div><div><span>Achievement</span><strong>${planned?`${achievement.toFixed(1)}%`:'N/A'}</strong></div></div><small class="gauge-note">Planned downtime is excluded from available production time.</small></div>`;
}
