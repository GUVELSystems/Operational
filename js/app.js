...
const HALF_RING_PLUGIN={id:'guvelHalfRingCenter',afterDraw(chart,args,opts){if(!opts||opts.value==null)return;const {ctx,chartArea}=chart;if(!chartArea)return;const x=(chartArea.left+chartArea.right)/2,y=chartArea.bottom-8;ctx.save();ctx.font='700 24px Inter, Arial, sans-serif';ctx.fillStyle=opts.color||'#14202b';ctx.textAlign='center';ctx.textBaseline='bottom';ctx.fillText(opts.text||'',x,y);ctx.restore();}};
...
function renderDashboardGeneral(){const box=document.getElementById('dashboardGeneral');if(!box)return;const cmp=dashboardComparison(),a=cmp.current,prev=cmp.previous,p=cmp.period;if(!a.d.prod.length){box.innerHTML='<div class="panel dashboard-empty"><div class="eyebrow">GENERAL</div><h2>No Data</h2><p>No production captures match the selected Dashboard filters.</p></div>';dashboardStatus('No Data in the selected filter scope.');return;}
 ...
 <div class="card kpi-card kpi-oee"><div class="kpi-top"><div class="label">OEE</div>${kpiGear('oee')}</div><div class="metric kpi-value-oee">${dashPct2(a.oee)}</div>${kpiDeltaMarkup('oee',a.oee,prev.oee,true,priorLabel)}</div>
 ...
 <div class="card kpi-card kpi-scrap"><div class="kpi-top"><div class="label">Scrap</div>${kpiGear('scrap')}</div><div class="metric kpi-value-scrap">${dashPct2(a.scrapPct)}</div>${kpiDeltaMarkup('scrap',a.scrapPct,prev.scrapPct,false,priorLabel)}</div>
 ...
 <div class="card kpi-card kpi-yield"><div class="kpi-top"><div class="label">Yield</div>${kpiGear('yield')}</div><div class="metric kpi-value-yield">${dashPct2(a.yieldRatio)}</div>${kpiDeltaMarkup('yield',a.yieldRatio,prev.yieldRatio,true,priorLabel)}</div>
 <div class="card kpi-card kpi-copq"><div class="kpi-top"><div class="label">COPQ</div>${kpiGear('copq')}</div><div class="metric kpi-value-copq">${dashPct2(a.copqPct)}</div>${kpiDeltaMarkup('copq',a.copqPct,prev.copqPct,false,priorLabel)}</div>
 </div>
