...
function productionGaugeMarkup(a){
  const av=a.availability==null?0:Math.max(0,Math.min(1,a.availability)), pe=a.performance==null?0:Math.max(0,Math.min(1,a.performance)), qu=a.quality==null?0:Math.max(0,Math.min(1,a.quality));
  const oee=a.oee==null?0:Math.max(0,Math.min(1,a.oee));
  return `<div class="production-gauge-card"><div class="production-gauge-title">OEE</div><div class="semi-gauge oee-gauge"><div class="semi-gauge-track"></div><div class="semi-gauge-fill" style="--gauge-p:${oee*100}%"></div><div class="semi-gauge-center"><strong>${gaugeValue(a.oee)}</strong><span>Overall Equipment Effectiveness</span></div></div><div class="gauge-legend"><span><i class="gauge-dot orange"></i>Availability <b>${gaugeValue(a.availability)}</b></span><span><i class="gauge-dot blue"></i>Performance <b>${gaugeValue(a.performance)}</b></span><span><i class="gauge-dot green"></i>Quality <b>${gaugeValue(a.quality)}</b></span></div></div>`;
}
function planVsActualMarkup(a){
  let planned=0,actual=a.production||0;
  for(const r of a.d.prod){const q=Number(r.production_quantity||0),sh=r.shifts,dur=shiftDurationSeconds(sh),excluded=Number(sh?.excluded_planned_minutes||0)*60,available=dur==null?0:Math.max(0,dur-excluded);const key=`${r.part_number_id||'unknown-part'}|${r.operation_id||'unknown-operation'}|${r.machine_id||'unknown-machine'}`,cfg=dashboardState.cycleTimes.find(x=>x.key===key),cy=Number(cfg?.cycle_time_seconds||r.operations?.ideal_cycle_time_seconds||0);if(available>0&&cy>0)planned+=available/cy;}
  const ratio=planned>0?Math.min(1.5,actual/planned):0,angle=-90+Math.min(1,ratio)*180;
  return `<div class="production-plan-card"><div class="production-gauge-title">Plan vs Actual</div><div class="odometer"><div class="odometer-track"></div><div class="odometer-pointer" style="transform:rotate(${angle}deg)"></div><div class="odometer-center"><strong>${planned?Math.round(actual).toLocaleString():'—'}</strong><span>Actual</span></div></div><div class="plan-actual-values"><div><span>Planned</span><strong>${planned?Math.round(planned).toLocaleString():'N/A'}</strong></div><div><span>Actual</span><strong>${actual.toLocaleString()}</strong></div><div><span>Achievement</span><strong>${planned?`${(actual/planned*100).toFixed(1)}%`:'N/A'}</strong></div></div><small class="gauge-note">Planned downtime is excluded from available production time.</small></div>`;
}
...
function liveLineStatusMarkup(a,p){
  const dayOnly=['Today','Previous Day'].includes(p.label)||['Today','Yesterday'].includes(p.label);
  if(dayOnly){const activeIds=new Set((dashboardState.sessions||[]).filter(s=>['RUNNING','ACTIVE','IN_PROGRESS'].includes(String(s.status||'').toUpperCase())).map(s=>s.machine_id));return `<div class="live-line-status"><div class="live-line-head"><h3>Live Line Status</h3><span>${activeIds.size} active</span></div><div class="live-line-grid">${dashboardState.machines.map(m=>`<div class="live-line-item ${activeIds.has(m.id)?'active':'inactive'}"><i></i><span>${escapeHtml(m.code||m.name||'Machine')}</span><b>${activeIds.has(m.id)?'ACTIVE':'IDLE'}</b></div>`).join('')||'<p class="muted">No machines registered.</p>'}</div></div>`;}
  const byDay=new Map();for(const r of a.d.prod){const day=String(r.production_date||'').slice(0,10);if(day)byDay.set(day,(byDay.get(day)||new Set()).add(r.machine_id));}const rows=[...byDay.entries()].sort((x,y)=>x[0].localeCompare(y[0]));return `<div class="live-line-status"><div class="live-line-head"><h3>Live Line Status</h3><span>Machines per day</span></div><div class="line-trend">${rows.map(([day,set])=>`<div><strong>${set.size}</strong><span>${day}</span></div>`).join('')||'<p class="muted">No daily machine activity.</p>'}</div></div>`;
}
