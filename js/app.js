...
function planVsActualMarkup(a){
  ...
  const needleColor=clamped<70?'#ff3131':clamped<80?'#f0b323':'#16a957';
  return `<div class="production-plan-card guvel-gauge-card guvel-plan-card"><div class="production-gauge-title-row"><div><div class="production-gauge-title">Plan vs Actual</div><div class="gauge-subtitle">Production achievement</div></div>${kpiGear('plan_actual')}</div><div class="guvel-odo-visual"><svg viewBox="0 0 360 250" role="img" aria-label="Plan vs Actual ${value}"><path d="${arc(180,360,132)}" class="guvel-odo-track"/><path d="${arc(180,306,132)}" class="guvel-odo-zone guvel-green"/><path d="${arc(306,324,132)}" class="guvel-odo-zone guvel-yellow"/><path d="${arc(324,360,132)}" class="guvel-odo-zone guvel-red"/><g class="guvel-odo-ticks">${ticks}</g><g class="guvel-odo-needle" style="transform:rotate(${angle+90}deg);--needle:${needleColor}"><line x1="180" y1="190" x2="180" y2="75"/><circle cx="180" cy="190" r="14"/></g><circle cx="180" cy="190" r="7" class="guvel-odo-hub"/><text x="180" y="145" text-anchor="middle" class="guvel-odo-value">${value}</text><text x="180" y="162" text-anchor="middle" class="guvel-odo-label">ACHIEVEMENT</text><text x="42" y="226" text-anchor="middle" class="guvel-odo-scale">0%</text><text x="180" y="35" text-anchor="middle" class="guvel-odo-scale">70%</text><text x="318" y="226" text-anchor="middle" class="guvel-odo-scale">100%</text></svg></div><div class="plan-actual-values premium"><div><span>Planned</span><strong>${planned?Math.round(planned).toLocaleString():'N/A'}</strong></div><div><span>Actual</span><strong>${actual.toLocaleString()}</strong></div><div><span>Achievement</span><strong>${planned?achievement.toFixed(1)+'%':'N/A'}</strong></div></div><small class="gauge-note">Planned downtime is excluded from available production time.</small></div>`;
}
