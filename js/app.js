...
async function renderStatusFoundation(){
  ...
  const [m,sh,c,p,people,operations,opMachines,sessions]=await Promise.all([
    ...
    sb.from('operations').select('id,part_number_id,operation_number,name').eq('company_id',activeCompanyId).order('operation_number'),
    sb.from('operation_machine_cycle_times').select('operation_id,part_number_id,machine_id').eq('company_id',activeCompanyId),
    sb.from('machine_production_sessions').select('*,machines(code,name),shifts(code,name),customers(code,name),part_numbers(part_number,description),operations(operation_number,name)').eq('company_id',activeCompanyId).eq('status','active').order('started_at')
  ]);
  const errors=[m,sh,c,p,people,operations,opMachines,sessions].filter(x=>x.error); if(errors.length){view.innerHTML=`<div class="panel"><h2>Status foundation not ready</h2><p>${escapeHtml(errors[0].error.message)}</p><p>Run SQL migration 017_phase_2_0_A_status_foundation.sql first, and confirm Phase 1.6.1 is installed.</p></div>`;return;}
  ...
  renderCards();
  stMachine.addEventListener('change',refreshOperationOptions);stPart.addEventListener('change',refreshOperationOptions);
  document.getElementById('statusStartForm').onsubmit=async e=>{e.preventDefault();const msg=document.getElementById('statusMsg');msg.textContent='Starting…';const payload={company_id:activeCompanyId,machine_id:stMachine.value,shift_id:document.getElementById('stShift').value,customer_id:document.getElementById('stCustomer').value,part_number_id:stPart.value,lot_number:document.getElementById('stLot').value.trim(),operation_id:stOperation.value||null,operator_id:document.getElementById('stOperator').value||null,supervisor_id:document.getElementById('stSupervisor').value||null,created_by:(await sb.auth.getUser()).data.user?.id||null};if(stOperation.required&&!payload.operation_id){msg.textContent='Select an operation for this part number.';msg.className='status error';return;}const r=await sb.from('machine_production_sessions').insert(payload);if(r.error){msg.textContent=r.error.message;msg.className='status error';return;}await renderStatusFoundation();};
}
