...
function planVsActualMarkup(a){
  ...
  const zones=segments.map(seg=>`<path d="${arc(180+seg.from*180,180+seg.to*180,132)}" class="guvel-odo-zone" stroke="${seg.color}"/>`).join('');
  const needleColor=kpiThresholdColor('plan_actual',achievement,achievement<.7?'#ff3131':achievement<.8?'#f2bd21':'#16a957');
...
function modelMixMarkup(a){const mix=new Map();for(const r of a.d.prod){const key=r.part_number_id||'unknown';const label=r.part_numbers?.part_number||key;mix.set(label,(mix.get(label)||0)+Number(r.production_quantity||0));}const rows=[...mix.entries()].sort((x,y)=>y[1]-x[1]);const total=rows.reduce((n,x)=>n+x[1],0);let acc=0;const stops=rows.map(([label,v],i)=>{const start=acc;acc+=total?v/total*100:0;return `${['#0cc0df','#ff3131','#143980','#006732','#f0a24b','#7b61ff'][i%6]} ${start}% ${acc}%`;}).join(',');return `<div class="model-mix-card"><div class="production-gauge-title">Model Mix</div><div class="pie-mix" style="background:conic-gradient(${stops||'#dce5e9 0 100%'})"></div><div class="mix-list">${rows.slice(0,6).map(([label,v],i)=>`<div><i style="background:${['#0cc0df','#ff3131','#143980','#006732','#f0a24b','#7b61ff'][i%6]}"></i><span>${escapeHtml(label)}</span><b>${total?(v/total*100).toFixed(1):0}%</b></div>`).join('')||'<p class="muted">No production data.</p>'}</div></div>`;}
