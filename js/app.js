...
function plantHexMarkup(x){
  ...
  return `<button type="button" class="plant-hex is-${x.state}" data-plant-machine="${escapeHtml(x.m.id)}" title="${tip}" aria-label="${escapeHtml(parts.join('. '))}"><i aria-hidden="true"></i><span><small>${escapeHtml(x.m.code||'—')}</small><b>${escapeHtml(label)}</b></span></button>`;
}
function plantLayoutHive(host,list){
  if(!host)return;const w=host.clientWidth||600,gap=6;const famSizes={};for(const x of list){const f=plantFamily(x.m.code);famSizes[f]=(famSizes[f]||0)+1;}const widest=Math.min(w<520?3:99,Math.max(2,...Object.values(famSizes)));const fit=Math.floor((w-(widest-1)*gap-gap/2)/(widest+.5));const hexW=Math.max(w<520?60:72,Math.min(w<520?96:124,fit));
