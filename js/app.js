...
function productionGaugeMarkup(a){
  ...
  const paths=vals.map((v,i)=>{const start=180+i*60,end=start+60,filled=start+60*v;return `<path class="oee-ring-track" d="${oeeArcPath(160,160,112,start,end)}"/><path class="oee-ring-value" stroke="${colors[i]}" d="${oeeArcPath(160,160,112,start,filled)}"/>`;}).join('');
  return `<div class="production-gauge-card"><div class="production-gauge-title">OEE</div><div class="oee-ring-wrap"><svg class="oee-ring" viewBox="0 0 320 178" role="img" aria-label="OEE ${gaugeValue(a.oee)}"><g>${paths}</g></svg><div class="oee-ring-center"><strong>${gaugeValue(a.oee)}</strong><span>Overall Equipment Effectiveness</span></div></div><div class="gauge-legend">${labels.map((l,i)=>`<span><i class="gauge-dot" style="background:${colors[i]}"></i>${l} <b>${gaugeValue([a.availability,a.performance,a.quality][i])}</b></span>`).join('')}</div></div>`;
}
